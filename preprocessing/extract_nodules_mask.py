...
class NoduleExtractor:
    ...
    def process_seriesuid(
        ...
        image_path = str(self.image_root / f"{seriesuid}.nii.gz")
        ...
        for i, coord in enumerate(coords):
            ...
            if Path(output_path).is_file():
                ...
                if Path(image_path).is_file():
                    ...
                    logging.info(f"Missing nii.gz file: {str(image_path)}")
...
if __name__ == "__main__":
    ...
    extractor = NoduleExtractor(
        csv_path=Path(
            "/data/luna25/data/LUNA25_Public_Training_Development_Data.csv"
        ),
        image_root=Path("/data/luna25/data/luna25_masks"),
        output_path=Path(
            "/data/luna25/data/luna25_masks_extracted"
        ),
        postfix="_0000",
        save_format=".nii.gz",
    )
