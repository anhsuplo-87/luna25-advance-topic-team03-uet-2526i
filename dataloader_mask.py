...
def normalize_age(age):
    ...
    age_norm = (age - 60) / 20      #TODO: thay bang mean va std
    age_norm = torch.clamp(age_norm, -1.5, 1.5)
    age_norm = torch.tensor([age_norm], dtype=torch.float32)

    return age_norm

def normalize_gender(gender):
    gender_norm = torch.tensor(0.0 if gender == "Male" else 1.0, dtype=torch.float32)
    gender_norm = gender_norm * 2.0 - 1.0
    gender_norm = torch.tensor([gender_norm], dtype=torch.float32) # (-1, 1)
...
class CTCaseDataset(data.Dataset):
    ...
    def __getitem__(self, idx):  # caseid, z, y, x, label, radius
        ...
        age = normalize_age(pd.Age_at_StudyDate)

        gender = normalize_gender(pd.Gender)
...
def test():
    ...
    train_loader = get_data_loader(
        ...
        mask_data_dir=config.MASK_DATADIR,
        dataset=dataset,
        mode=config.MODE,
        workers=8,
    ...
    for i, data in enumerate(train_loader):
        print(i, data["image"].shape, data["mask"].shape, data["label"].shape, data["age"].shape, data["gender"].shape)
